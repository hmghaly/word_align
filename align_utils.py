...
def get_index_matching(src_tokens0,trg_tokens0,src_index0,trg_index0,max_phrase_length=5): #match src/trg phrase based on index matching intersection/ratio
  ...
  for src_phrase0,src_locs_indexes0 in src_phrase_index_loc_dict.items():
    ...
    for trg_phrase0,trg_locs_indexes0 in trg_phrase_index_loc_dict.items():
      ...
      matching_list.append((src_phrase0,trg_phrase0,src_locs0,trg_locs0,intersection1,round(adj_ratio,4)))
  ...
  for a in matching_list:
    ...
    valid=False
    if src_tokens0==len(src_locs0) or trg_check==len(trg_locs0): valid=True
    if src_check>0 and trg_check>0: valid=True #final_matching_list.append(a)
