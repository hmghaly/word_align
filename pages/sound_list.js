function makeSoundPage(stage) {
    var stageW = stage.width;
    var stageH = stage.height;
    var page = new Page(stageW, stageH, green,red);
    STYLE = {font:"reuben", size:50};
    
    new Label({color:purple, text:"Sound\nList", size:50,variant:true, align:CENTER}).pos(0,30,CENTER,TOP,page);
...
//        }
        ...
        y0=0
        y_offset=30
        zim.loop(game_data.sound_ids,function(item,i){
            //console.log(item,i)
            cur_obj=game_data.sound_dict[item]
//            shapes=game_data.shape_dict[item]
//            shapes.reverse();
//            name=game_data.name_dict[item]
            shapes=[cur_obj.arabic]
            name=cur_obj.sound
            //console.log(name,shapes)
            let rect0 = new Rectangle(stageW*0.8,110,colors)
            rect0.id=item
            name_ml=multiline(name,5)
            //new Label({text:name_ml, color:black,  size:30}).center(rect0)
            new Label({text:name_ml, color:black,  size:45}).pos(30,0, LEFT,CENTER,rect0)
            
            //circle.pos(30,y0,page.win)
            x0=stageW*0.3
            objects.push(rect0);
            for (const sh of shapes){
                let circle = new Circle(50, purple);
